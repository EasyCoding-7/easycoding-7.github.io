---
layout: post
title:  "(C++ : Perfect-Forwarding-2) Perfect Forwarding 기본 구현"
summary: ""
author: C++
...
#keywords: ['unique_ptr']
usemathjax: true
permalink: /blog/cpp/perfect-forwarding/2/
---
...
// 핵심 1. 함수 인자를 받을 때 reference를 사용해야한다.
{
  f(arg);
}
...
이런 질문이 생긴다 그럼 `foo`는? (`void foo(int a) { cout << "foo" << endl; }`)<br>
`foo`의 매개 변수는 `(int a)` lvalue인데?? (다음강에서 해결됨. -> forwarding reference)